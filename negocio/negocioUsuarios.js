    ...
    coleccionUsuarios
      ...
      .then((usuarioEncontrado) => {
        if (!usuarioEncontrado) {
          reject({
            codigo: 404,
            mensaje: "No existe un usuario con este login y password",
          });
          return;
        }
        //delete usuarioEncontrado.password
...
exports.modificarUsuario = function (usuario, autoridad) {
  return new Promise(function (resolve, reject) {
    ...
    if (autoridad.rol == "CLIENTE" && autoridad._id != usuario._id) {
      reject({
        codigo: 403,
        mensaje: "Los clientes solo pueden modificarse a si mismos",
      }); //Mal
      return;
    }
...
exports.bajaUsuario = function (idUsuario, autoridad) {
  return new Promise(function (resolve, reject) {
    ...
    if (autoridad.rol == "CLIENTE" && autoridad._id != idUsuario) {
      reject({
        codigo: 403,
        mensaje: "Un cliente solo puede darse de baja a si mismo",
      });
      return;
    }

    let coleccionUsuarios = process.esquema.collection("usuarios");
    let coleccionUsuariosHistorico =
      process.esquema.collection("usuarios_historico");
    ...
    coleccionUsuarios
      ...
      .then((usr) => {
        ...
        return coleccionUsuarios.findOneAndDelete({
